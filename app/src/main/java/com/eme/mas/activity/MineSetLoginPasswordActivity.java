...
/**
 * 设置登录密码 (从设置里进入的修改密码)
 * Created by dijiaoliang on 16/7/28.
 */
@WLayout(layoutId = R.layout.activity_set_login_password)
public class MineSetLoginPasswordActivity extends BaseActivity {

    private final static String TAG = MineSetLoginPasswordActivity.class.getSimpleName();
    ...
    public void done() {
        if (!isCurrentPassword(password) || !isCurrentPassword(passwordAgain)) {
            ...
            if (NetworkStatusUtil.isNetworkConnected(MineSetLoginPasswordActivity.this)) {
                isHideLayer(llAvLoadingTransparent44,false);
                iUserInfo.setLoginPassword(password, phoneNumber, TAG);
            } else {
                ToastUtil.shortToast(MineSetLoginPasswordActivity.this, R.string.net_error);
            }
        }
    }
    ...
    public void onActionSucc(Result result) {
        ...
        if (WAction.SET_LOGIN_PASSWORD.equals(action)) {
            ...
            intent.putExtra("flag",WValue.MAIN_FRAGMENT_MINE);
